...
git diff -- examples/shop/internal/payment/handler.go | ./bin/aquila experiment -fixture -n 1 -out out/evidence.json
```
...
The shop is the reference system and the only Compose env Aquila can prepare. Export OTLP from the **running process** (the shop Compose already sets `OTEL_EXPORTER_OTLP_ENDPOINT`) into the collector `make dev` starts. The collector does not ingest a codebase.
