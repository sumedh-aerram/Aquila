...
Aquila is meant to sit beside work in a different repo—not only `examples/shop`. Switch by changing directory. There is no target profile.

The shop is the reference system and the only Compose env Aquila can prepare. Export OTLP from the **running process** to the collector on loopback (`OTEL_EXPORTER_OTLP_ENDPOINT=127.0.0.1:4317`). Shop Compose uses `otel-collector:4317` on the Docker network. The collector does not ingest a codebase. Health-probe traces (`/healthz` and friends) are omitted from the observe/impact window so another app is not drowned by shop liveness checks. The span store is still shared: hops from shop and from your app can appear in the same window. Locate only binds spans whose `code.function.name` and `code.file.path` uniquely match **your** module.
...
3. Start **two** isolated gateways yourself, or pass `-base`/`-patch` to gateways you already run. `aquila experiment` without those flags prepares shop Compose only and will not boot Libra.
...
`-dir` is the module and git work tree under change so impact and the stored run record **that** HEAD, not Aquila’s. If the other app does not set `code.function.name` and `code.file.path` on spans, locate stays unmapped and impact runtime stays empty. That is correct. Match is not a pass. Stored runs have `validated=false`.
